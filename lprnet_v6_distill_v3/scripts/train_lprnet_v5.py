...
class LPRNetV5(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES, dropout_rate=0.3):
        ...
        self.container = nn.Sequential(
            nn.Conv2d(num_classes + 256, num_classes, 1, bias=False),
    ...
    def _init_container(self):
        with torch.no_grad():
            ...
            nn.init.xavier_normal_(w[:, :self.container[0].in_channels - 256, :, :])
            nn.init.zeros_(w[:, self.container[0].in_channels - 256:, :, :])
...
def load_v4_pretrained(model, pretrained_path):
    ...
    v4_to_v5_backbone = {}
    for i in range(9):
        v4_to_v5_backbone[i] = i
    v4_to_v5_backbone[10] = 11
    v4_to_v5_backbone[11] = 12
    v4_to_v5_backbone[14] = 15

    new_state = {}
    loaded = 0
    skipped = 0

    for v4_key, v4_val in pretrained_state.items():
        if v4_key.startswith('backbone.'):
            parts = v4_key.split('.')
            layer_idx = int(parts[1])

            if layer_idx in v4_to_v5_backbone:
                v5_layer_idx = v4_to_v5_backbone[layer_idx]
                parts[1] = str(v5_layer_idx)
                v5_key = '.'.join(parts)

                if v5_key in model_state and v4_val.shape == model_state[v5_key].shape:
                    new_state[v5_key] = v4_val
                    loaded += 1
                else:
                    skipped += 1
            else:
        ...
        elif v4_key.startswith('container.'):
            if v4_key in model_state and v4_val.shape == model_state[v4_key].shape:
                new_state[v4_key] = v4_val
                loaded += 1
            else:
                skipped += 1
        else:
    ...
    print(f'  V4→V5权重映射: 匹配{loaded}层, 跳过{skipped}层')
    print(f'  SpatialAttention(新): 随机初始化')
    print(f'  container跳连接: 部分继承+重新初始化')
    print(f'  预训练Epoch: {checkpoint.get("epoch", "unknown") + 1}')
    print(f'  预训练准确率: {checkpoint.get("best_acc", "unknown"):.4f}')
...
class Trainer:
    ...
    def train(self, start_epoch=0):
        ...
        print(f'V5改进: 旋转±20° | 省份辅助分类损失 | 空间注意力(k=3) | 跳连接(container) | 省份遮挡增强')
        print('-' * 50)
...
def main():
    ...
    _save_dir = os.environ.get("V5_OUTPUT_DIR", "./outputs/lprnet_v5")
    config = {
    ...
    model = LPRNetV5(num_classes=NUM_CLASSES, dropout_rate=config['dropout_rate'])
    ...
    elif config.get('pretrained_model') and os.path.exists(config['pretrained_model']):
        load_v4_pretrained(model, config['pretrained_model'])
